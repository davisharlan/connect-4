...
const Column = () => {
  ...
  const column = [...Array(6).keys()].reverse().map((value) => <Cell cellState = {columnState[value]}></Cell>);

  return <div className="column" onClick={() => setColumnState(fillUnoccupiedCellFromBottom(columnState))}>{column}</div>;
};
